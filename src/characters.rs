...
impl _CharacterMaps {
    ...
    {
        _CharacterMaps { maps : Vec::<CharacterMap>::new() }
    }
...
impl CharacterMaps {
    ...
    {
        CharacterMaps { is_utf8 : true, maps : Rc::new(_CharacterMaps::empty())}
    }
...
impl CharacterMap {
    ...
    fn new(attributes : & Vec<OwnedAttribute>) -> CharacterMap
    {
        ...
        CharacterMap { id : id, bytes_per, chars : HashMap::<u16, Character>::new() }
    }
    ...
    fn get_unicode(&self, ch : u16) -> String
    {
        match self.chars.get(&ch) {
            ...
            None => { 
                self.display();
                panic!("Failed to find {} in character map {} size {}", ch, self.id, self.bytes_per)
            }
        }.get_unicode()
    }

    fn display(&self) {
        println!("Character Map {}, size of chars {}", self.id, self.bytes_per);
...
impl Character {
    ...
    {
        Character { unicode : unicode, count : 0 }
    }
...
pub fn read_character_file(filepath : &str) -> CharacterMaps
{
    ...
    return CharacterMaps { is_utf8 : false, maps : Rc::new(_CharacterMaps::new(maps))};
}
