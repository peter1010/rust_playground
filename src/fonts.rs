    ...
    pub fn get_glyph(&self, char_map : u8, font_family : u8, codepoint : u16) -> Option<Vec::<u8>> {
        for section in self.sections.iter() {
            if (section.char_map == char_map) &&
                ...
                let idx : usize = ((codepoint - section.min_codepoint) as usize) * (section.bytes_per_glyph as usize);
                let mut glyph = Vec::<u8>::new();
                glyph.extend_from_slice(&section.blob[idx..(idx + (section.bytes_per_glyph) as usize)]);
                return Some(glyph);
            }
        }
        None
...
impl FontSection {
    ...
    pub fn from(fp : & mut File) -> io::Result<FontSection> {
        ...
        println!("map ={}, id = {}, {} x {}, {} to {}", char_map, font_family, glyph_width, glyph_height,
            min_codepoint, max_codepoint);
        let mut blob_size : usize = (bytes_per_glyph as usize) * ((max_codepoint - min_codepoint + 1) as usize);
        let mut buf = [0; 512];
        ...
        Result::Ok(FontSection { char_map, font_family, min_codepoint, max_codepoint, glyph_width, glyph_height, bytes_per_glyph, blob})
    }
}
